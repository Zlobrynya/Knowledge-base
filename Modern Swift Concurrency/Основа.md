...
Task.detached(executorPreference: customExecutor) {
    print("Работа задачи начинается на потоке: \(Thread.currentThread) 1 ")
    try? await Task.sleep(nanoseconds: 100)
    print("Работа задачи продолжается на потоке: \(Thread.currentThread) 2 ")
}
Task.detached(executorPreference: customExecutor) {
    print("Работа задачи начинается на потоке: \(Thread.currentThread) 3 ")
    try? await Task.sleep(nanoseconds: 100)
    print("Работа задачи продолжается на потоке: \(Thread.currentThread) 4")
}
```
...
Так как все проверки происходят на swift 6, для вывода на каком потоке сейчас работает код, сделана обертка позволяющая обойти проверку на strict concurrency, далее эта обертка будет использовать во всех тестах когда надо узнать тип потока.
```swift
...
В этом разделе мы затронем тему выбора executor на котором будет выполняться задача, а так же его наследование в Structured и Unstructured подходах. 
Swift напрямую не может выдать нам executor на котором будет выполняться задача, поэтому мы будем проверять по косвенным признакам через `Thread.current` и заодно посмотрим как работает Cooperative Thread Pool в контексте точек остановки.
Начнем с простого примера:
```swift
Task {
    print("Start \(Thread.currentThread)")
    try await Task.sleep(for: .seconds(0.1))
    print("End \(Thread.currentThread)")
}
__
Вывод в консоль:
Start <NSThread: 0x600001704c40>{number = 6, name = (null)}
End <NSThread: 0x6000017041c0>{number = 9, name = (null)}
```
Как видно тут у нас все выполняется на бекграунд потоке и обратите внимание, что после остановки у нас тред отличается от того, на котором таска началась. 
А теперь добавим анотацию MainActor и посмотрим что произойдет
```swift
Task { @MainActor in
...
__
Вывод в консоль:
Start <_NSMainThread: 0x600001704000>{number = 1, name = main}
End <_NSMainThread: 0x600001704000>{number = 1, name = main}
```
Как видно, таска началась на Main потоке и после точке остановки вернулась на тот же самый Main поток. 
Теперь немного усложним наш код и добавим вместо sleep отдельный async метод, в данном примере последуем Structured подходу.
```swift
Task {
    print("Start \(Thread.current.description)")
    try await anotherAsyncMethod()
    print("End \(Thread.current.description)")
}

func anotherAsyncMethod() async throws {
    print("Start anotherAsyncMethod \(Thread.current.description)")
    try await Task.sleep(for: .seconds(0.1))
    print("End anotherAsyncMethod \(Thread.current.description)")
}
__
Вывод в консоль:
Start <NSThread: 0x600001707200>{number = 5, name = (null)}
Start anotherAsyncMethod <NSThread: 0x600001707200>{number = 5, name = (null)}
End anotherAsyncMethod <NSThread: 0x6000017309c0>{number = 6, name = (null)}
End <NSThread: 0x6000017309c0>{number = 6, name = (null)}
Или
Start <NSThread: 0x600001714040>{number = 5, name = (null)}
Start anotherAsyncMethod <NSThread: 0x600001714040>{number = 5, name = (null)}
End anotherAsyncMethod <NSThread: 0x600001714040>{number = 5, name = (null)}
End <NSThread: 0x600001714040>{number = 5, name = (null)}
```
Как видно из вывода с консоли мы можем получить 2 разные ситуации, когда у нас все выполняется на одной потоке и когда идет переключение потоков, это все результат работы Global concurrent executor и загруженности потоков.
А теперь разбавим этот код MainActor
```swift
Task { @MainActor in
    print("Start \(Thread.current.description)")
    try await anotherAsyncMethod()
...
__
Вывод в консоль:
Start <_NSMainThread: 0x600001710040>{number = 1, name = main}
Start anotherAsyncMethod <NSThread: 0x600001716e00>{number = 3, name = (null)}
End anotherAsyncMethod <NSThread: 0x600001716e00>{number = 3, name = (null)}
End <_NSMainThread: 0x600001710040>{number = 1, name = main}
```
И тут мы начинаем наблюдать следующую картину, Task как и в простом варианте начинается на Main потоке (Main Actor executor), но при выполнении нашего async метода переключается на фоновые потоки (Global concurrent executor), и при завершении нашего async метода возврщается обратно в Main поток. 
И как следствие мы можем сделать следующий вывод: 
"Вызов async метода, будет всегда работать на Global concurrent executor, если принудительно не указан actor на котором следует выполнять"
Это был бы корректный вывод для swift версии **до** **6.2**, в версии `6.2` будет изменено это поведение, более подробнее описано в этом [проползале](https://github.com/swiftlang/swift-evolution/blob/main/proposals/0461-async-function-isolation.md)
**Ниже будет теоретические выводы основываясь на этом проползале.**
Возьмем выше описанный код и мысленно прогоним через выполнение 
```swift
...
__
Вывод в консоль:
Start <_NSMainThread: 0x600001710040>{number = 1, name = main}
Start anotherAsyncMethod <_NSMainThread: 0x600001710040>{number = 1, name = main}
End anotherAsyncMethod <_NSMainThread: 0x600001710040>{number = 1, name = main}
End <_NSMainThread: 0x600001710040>{number = 1, name = main}
```
И тут мы обнаруживаем, что наш async метод, начинает обрабатываться на Main потоке. Все из за того, что теперь async методы будут **по умолчанию** обрабатываться на том actor executer в котором была запущена изначальная Task. В нашем случае это MainActor. Что б этого избежать и отправить наш метод на Global concurrent executor, проползал добавляет атрибут `@concurrent`. Т.е наш код на **swift 6.2**  должен выглядить так:
```swift
Task { @MainActor in
    ...
    try await anotherAsyncMethod()
...
```
Теперь давайте рассмотрим ситуация с Unstructured, и начнем с простого примера с созданием отдельной Task
```swift
Task { @MainActor in
    print("Start \(Thread.currentThread.description)")
    Task {
        print("Start internal Task \(Thread.currentThread.description)")
        try await anotherAsyncMethod()
        print("Stop internal Task \(Thread.currentThread.description)")
    }
    print("End \(Thread.currentThread.description)")
}

func anotherAsyncMethod() async throws {
    print("Start anotherAsyncMethod \(Thread.currentThread.description)")
    try await Task.sleep(for: .seconds(0.2))
    print("End anotherAsyncMethod \(Thread.currentThread.description)")
}
__
Вывод в консоль:
Start <_NSMainThread: 0x60000170c000>{number = 1, name = main}
End <_NSMainThread: 0x60000170c000>{number = 1, name = main}
Start internal Task <_NSMainThread: 0x60000170c000>{number = 1, name = main}
Start anotherAsyncMethod <NSThread: 0x60000172d800>{number = 10, name = (null)}
End anotherAsyncMethod <NSThread: 0x60000172d800>{number = 10, name = (null)}
Stop internal Task <_NSMainThread: 0x60000170c000>{number = 1, name = main}
```
Как видно по output, наша Task, которую мы создали внутри родительской Task, унаследовала актор в котором она создалась, т.е MainActor, и начала работать на Main потоке, а дальше все по той же логики, что описывалась выше, метод уже начинает работать на глобальном экзекьютере. Что же касается swift 6.2, то там будет такое же поведение как было описано выше.
А теперь рассмотрим вариант с `Task.detached`
```swift
Task { @MainActor in
    print("Start \(Thread.currentThread.description)")
    Task.detached {
...
Вывод в консоль:
Start <_NSMainThread: 0x600001710040>{number = 1, name = main}
Start internal Task <NSThread: 0x60000170a3c0>{number = 2, name = (null)}
Start anotherAsyncMethod <NSThread: 0x60000170a3c0>{number = 2, name = (null)}
End <_NSMainThread: 0x600001710040>{number = 1, name = main}
End anotherAsyncMethod <NSThread: 0x600001711f40>{number = 3, name = (null)}
Stop internal Task <NSThread: 0x600001711f40>{number = 3, name = (null)}
```
Как и ожидалось, так как Task у нас detached, она не наследует ничего с родительской задачи и создается обособленно и в виду того, что мы не указали на каком акторе ей начать выполняться, она по умолчанию начинает работать на глобальном акторе.
